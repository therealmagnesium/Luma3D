...
namespace Graphics
{
    ...
    {
        Camera* camera = Renderer.primaryCamera;
    ...
    {
        // TODO: Make the camera orbit around the target
    }

    void UpdateCamera(CameraType type)
    {
        if (Renderer.primaryCamera != NULL && !Renderer.primaryCamera->isLocked)
        {
            switch (type)
            {
                case CAMERA_FREE:
